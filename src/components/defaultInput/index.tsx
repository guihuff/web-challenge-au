...
export function Input({ label, isRequired, ...rest }: InputProps) {
  return (
    <label className='flex flex-col text-sm my-1 leading-6'>{isRequired ? `${label} *`: label} 
...
export function Select({ name, isRequired, options, ...rest }: SelectProps) {
  return (
    <label className='flex flex-col text-sm my-1 leading-6'>{isRequired ? `${name} *`: name} 
